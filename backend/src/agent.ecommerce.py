...
async def list_products(ctx: RunContext, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Retrieves and filters the product catalog (listings) based on provided criteria.
    Args:
...
async def add_item_to_cart(ctx: RunContext, product_id: str, quantity: int = 1, size: Optional[str] = None, color: Optional[str] = None) -> Dict[str, Any]:
    """
    Adds a specified quantity of a product (intended for 'Buy It Now' items) to the global active shopping cart (ACTIVE_CART).
...
async def create_order(ctx: RunContext, items_to_purchase: List[Dict[str, Any]] = []) -> Dict[str, Any]:
    """
    Finalizes the purchase (checkout). Uses the provided items_to_purchase list OR the global ACTIVE_CART.
...
class Assistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            instructions="""You are **Ezra**, a helpful and enthusiastic voice shopping agent for **eBay**. Your primary goal is to help the user browse the diverse listings, manage their shopping cart for 'Buy It Now' items, and finalize their purchases using your available tools.

            **Persona & Tone:** Be engaging, knowledgeable about a vast array of unique items, and focused on finding the best deals and items, reflecting the dynamic nature of eBay.

            **Flow:**
            1. **Browsing:** Use `list_products` with filters (category, price, color) to find suitable listings. Summarize results clearly, numbering them.
            
            2. **Cart Management (CRITICAL):**
                * When a user indicates a desire to buy an item (a 'Buy It Now' style purchase), use **`add_item_to_cart`**.
            ...
            **Assistant:** "Welcome to eBay! I'm Ezra, your personal shopping agent. I can help you search through millions of listings. What unique item or great deal can I help you find today?"
            """,
